    ...
    return (
        <div>
            ...
            <div>
                ...
                <button onClick={() => {
                    //입력된 값을 제목 배열에 추가
                    //입력된 값 : input 요소에서 onChange가 발생할 때 >> inputText state 변수에 저장되어 있음.
                    //제목 배열 news 배열변수에 추가 >> 리렌더링 >> news배열의 갯수만큼 반복(map) 화면에 표시
...

            {
                news.map((item, index) => {   // 0 1 2 
                    ...
                    return (
                        <div className='post-list' id="input news" value={inputText} key={index}>
                            <h4 onClick={() => {
